...
export function ContentActions({
  ...
  return (
    <div className="space-y-3 ">
      <div className={`grid gap-2 ${isOwner ? "grid-cols-2" : "grid-cols-3"}`}>
        ...
        <form action={saveAction}>
          ...
          <Button
            ...
            className={isSaved ? "text-primary-strong" : ""}>
            {savePending ? (
              <LoaderCircle className="size-4 animate-spin" />
            ) : (
        ...
        {!isOwner ? (
          <ButtonLink
            href={`/improvements/new?type=${targetType}&targetId=${targetId}`}
            variant="outline"
            fullWidth
            title={`${improvements.toLocaleString("fa-IR")} بهبود پذیرفته‌شده`}>
        ...
        <form
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div
              ...
              aria-label="امتیاز از یک تا پنج">
              {[5, 4, 3, 2, 1].map((value) => (
                <button
                  key={value}
                  type="submit"
                  ...
                  className="rounded-md p-1 text-faint transition hover:text-warning focus-visible:text-warning"
                  aria-label={`${value.toLocaleString("fa-IR")} ستاره`}>
                  <Star
                    className={`size-4 ${selectedRating && value <= selectedRating ? "fill-warning text-warning" : ""}`}
                  />
                </button>
              ))}
            </div>
      ...
      {[likeState, saveState, ratingState].find(
