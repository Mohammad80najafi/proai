...
export function ChatClient({ conversationId, currentUserId, participant, initialMessages }: { conversationId: string; currentUserId: string; participant: ChatUser; initialMessages: ChatMessage[] }) {
  ...
  useEffect(() => {
    const socket = io(process.env.NEXT_PUBLIC_REALTIME_URL ?? "http://localhost:3001", { withCredentials: true, transports: ["websocket", "polling"] });
    socketRef.current = socket;
    socket.on("connect", () => { setConnected(true); socket.emit("conversation:join", { conversationId }); });
    socket.on("disconnect", () => setConnected(false));
    socket.on("message:new", (message: ChatMessage) => setMessages((items) => upsert(items, message, currentUserId)));
    socket.on("typing", (event: { conversationId: string; userId: string; active: boolean }) => { if (event.conversationId === conversationId && event.userId === participant.id) setTyping(event.active); });
  ...
  }, [conversationId, currentUserId, participant.id]);
  ...
  function announceTyping(value: string) {
    ...
    typingTimer.current = setTimeout(() => socketRef.current?.emit("typing", { conversationId, active: false }), 1_400);
  }
  ...
  return (
    <section className="flex h-[calc(100vh-9.5rem)] min-h-[560px] flex-col overflow-hidden rounded-2xl border border-white/[0.07] bg-surface">
      <header className="flex h-16 items-center justify-between border-b border-white/[0.06] px-4 sm:px-5"><div className="flex items-center gap-3"><Avatar src={participant.avatar} fallback={participant.displayName} alt={participant.displayName} size="sm" /><div><h2 className="text-sm font-semibold">{participant.displayName}</h2><p className="mt-0.5 flex items-center gap-1 text-[10px] text-faint"><Circle className={`size-2 ${connected ? "fill-success text-success" : "fill-faint text-faint"}`} />{connected ? "ارتباط زنده" : "حالت پشتیبان"}</p></div></div></header>
      <div className="scrollbar-subtle flex-1 space-y-3 overflow-y-auto p-4 sm:p-5" aria-live="polite">{messages.map((message) => <div key={message.id} className={`flex ${message.own ? "justify-start" : "justify-end"}`}><div className={`max-w-[82%] rounded-2xl px-4 py-2.5 text-sm leading-7 ${message.own ? "rounded-tr-md bg-primary text-white" : "rounded-tl-md border border-white/[0.07] bg-white/[0.045] text-slate-200"}`}><p className="whitespace-pre-wrap" dir="auto">{message.content}</p><span className={`mt-1 flex items-center justify-end gap-1 text-[9px] ${message.own ? "text-white/60" : "text-faint"}`}>{formatRelativeDate(message.createdAt)}{message.own ? <CheckCheck className="size-3" /> : null}</span></div></div>)}{typing ? <div className="flex justify-end"><div className="rounded-2xl rounded-tl-md bg-white/[0.045] px-4 py-3 text-xs text-faint">در حال نوشتن<span className="animate-pulse">…</span></div></div> : null}<div ref={endRef} /></div>
