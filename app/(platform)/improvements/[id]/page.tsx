...
export default async function ImprovementDetailPage({ params }: Props) {
  ...
  return (
    <div className="space-y-7">
      ...
      {request.hasBaseConflict ? <div className="flex gap-3 rounded-2xl border border-danger/20 bg-danger/[0.07] p-4 text-sm leading-7 text-danger"><AlertTriangle className="mt-1 size-5 shrink-0" /><p>نسخه اصلی پس از ساخت این فورک تغییر کرده است. پیشنهاد باید پیش از پذیرش با نسخه تازه هماهنگ شود.</p></div> : null}
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_340px] xl:items-start">
        <div className="space-y-6">
          <Card className="overflow-hidden"><div className="flex items-center justify-between border-b border-white/[0.06] px-5 py-4"><h2 className="font-semibold">نسخه پیشنهادی</h2><div className="flex flex-wrap gap-1.5">{request.changedPaths.map((path) => <Badge key={path}>{path}</Badge>)}</div></div><div className="p-5 sm:p-7"><Markdown>{mainText}</Markdown></div></Card>
        ...
        <aside className="space-y-4 xl:sticky xl:top-24">
          {request.isOwner ? <Card className="p-5"><h2 className="mb-4 font-semibold">تصمیم مالک</h2><ImprovementDecisionPanel requestId={request.id} status={request.status} /></Card> : null}
