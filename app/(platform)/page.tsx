...
export default async function HomePage() {
  ...
  if (!leadStory) {
    return (
      <div className="grid min-h-[55vh] place-items-center text-center">
        <div>
          <Radio className="mx-auto size-8 text-faint" />
          <h1 className="mt-5 text-3xl font-black">
            اتاق خبر در حال آماده‌سازی است
          </h1>
          <p className="mt-3 text-sm text-muted">
            به‌زودی خبرهای تازه اینجا منتشر می‌شوند.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-20 overflow-hidden pb-10 sm:space-y-28">
          ...
          <div className="relative h-full min-h-[558px] overflow-hidden rounded-[calc(2rem-0.375rem)] sm:min-h-[628px]">
            ...
            <div className="absolute inset-x-0 bottom-0 p-6 sm:p-10 lg:p-12">
              <div className="mb-5 flex flex-wrap items-center gap-2">
                <span
                  className={`rounded-full px-3 py-1.5 text-[10px] font-bold text-[#07110d] ${leadStory.accent}`}>
                  داستان جلد
                </span>
                <span className="rounded-full bg-black/35 px-3 py-1.5 text-[10px] text-slate-200 ring-1 ring-white/15">
                  {leadStory.category}
                </span>
              </div>
              ...
              <p className="pretty-text mt-5 max-w-2xl text-sm leading-8 text-slate-300 sm:text-base">
                {leadStory.summary}
              </p>
        ...
        <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-1">
          {secondaryStories.map((story) => (
            <Link
              key={story.slug}
              href={`/news/${story.slug}`}
              className="group flex min-h-[280px] flex-col overflow-hidden rounded-[1.75rem] bg-white/[0.04] p-1.5 ring-1 ring-white/[0.06] xl:min-h-0">
              <div className="relative min-h-40 flex-1 overflow-hidden rounded-[calc(1.75rem-0.375rem)] bg-[#0a0f17]">
                ...
                <span
                  className={`absolute end-4 top-4 size-3 rounded-full ${story.accent}`}
                />
              </div>
              <div className="px-3 pb-4 pt-5 sm:px-5">
                ...
                <h3 className="pretty-text mt-2 text-lg font-bold leading-8">
                  {story.title}
                </h3>
                <div className="mt-4 flex items-center justify-between gap-3">
                  <ReadingMeta date={story.date} readTime={story.readTime} />
                  <ArrowLeft
                    className="size-4 text-faint transition-transform duration-700 ease-[cubic-bezier(0.32,0.72,0,1)] group-hover:-translate-x-1 group-hover:text-white"
                    strokeWidth={1.4}
                    aria-hidden="true"
                  />
                </div>
      ...
      <section
        className="home-reveal home-reveal-delay-2"
        aria-labelledby="briefing-title">
        <div className="mb-7 flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="text-[10px] font-semibold tracking-[0.17em] text-primary-strong">
              THE BRIEFING
            </p>
            <h2
              id="briefing-title"
        ...
        <div className="scrollbar-subtle -mx-4 flex snap-x snap-mandatory gap-4 overflow-x-auto px-4 pb-5 sm:-mx-6 sm:px-6 xl:-mx-8 xl:px-8">
          {homeNewsItems.map((story) => (
            <Link
              key={story.slug}
              href={`/news/${story.slug}`}
              className="group w-[82vw] max-w-[390px] shrink-0 snap-start overflow-hidden rounded-[1.5rem] bg-white/[0.04] ring-1 ring-white/[0.06] sm:w-[360px]">
              <div className="relative aspect-[16/9] overflow-hidden bg-[#0a0f17]">
                <Image
                  ...
                  fill
                  sizes="390px"
                  className="object-cover opacity-75 transition-transform duration-1000 ease-[cubic-bezier(0.32,0.72,0,1)] group-hover:scale-105"
                />
                <div
                  className={`absolute inset-x-0 bottom-0 h-1 ${story.accent}`}
                />
              </div>
              <div className="p-5">
                <p className="text-[10px] font-semibold text-primary-strong">
                ...
                <h3 className="pretty-text mt-2 text-base font-bold leading-7">
                  {story.title}
                </h3>
                <p className="pretty-text mt-3 text-xs leading-6 text-muted">
                  {story.summary}
                </p>
              </div>
            </Link>
      ...
      <section
        id="latest"
        className="home-reveal home-reveal-delay-2 scroll-mt-24"
        ...
        <div className="grid gap-12 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-start lg:gap-16">
          <div>
            <div className="mb-7 flex items-center justify-between gap-4 border-b border-white/[0.08] pb-5">
              <div>
                <p className="text-[10px] font-semibold tracking-[0.16em] text-primary-strong">
                  LATEST STORIES
                </p>
                <h2
                  id="latest-title"
            ...
            <div className="divide-y divide-white/[0.08]">
              {[...feedStories, ...secondaryStories].map((story) => (
                <article key={story.slug} className="py-7 first:pt-0">
                  <Link
                    href={`/news/${story.slug}`}
                    className="group grid gap-5 sm:grid-cols-[220px_minmax(0,1fr)] sm:items-center">
                    <div className="relative aspect-[16/10] overflow-hidden rounded-2xl bg-[#0a0f17]">
                      ...
                      />
                      <span
                        className={`absolute end-3 top-3 size-2.5 rounded-full ${story.accent}`}
                      />
                    </div>
                    <div>
                      <p className="text-[10px] font-semibold text-primary-strong">
                      ...
                      <h3 className="pretty-text mt-2 text-xl font-black leading-9 tracking-[-0.025em] transition-colors duration-500 group-hover:text-white sm:text-2xl">
                        {story.title}
                      </h3>
                      <p className="pretty-text mt-3 text-sm leading-7 text-muted">
                        {story.summary}
                      </p>
                      <div className="mt-4 flex items-center justify-between gap-3">
                        <ReadingMeta
          ...
          <aside className="space-y-5 lg:sticky lg:top-24">
                ...
                {newsStories.slice(0, 4).map((story, index) => (
                  <li key={story.slug}>
                    <Link
                      ...
                      <span className="font-mono text-xl font-bold text-white/15">
                        {String(index + 1).padStart(2, "0")}
                      </span>
                      <div>
                        <p className="text-[10px] text-primary-strong">
                          {story.source}
                        </p>
                        <h3 className="pretty-text mt-1 text-xs font-semibold leading-6 text-slate-300 transition-colors duration-500 group-hover:text-white">
                          {story.title}
                        </h3>
                      </div>
                    </Link>
            ...
            <div className="overflow-hidden rounded-[1.5rem] bg-[#caffdf] p-6 text-[#07110d]">
              ...
              <h2 className="mt-9 text-2xl font-black leading-9">
                خلاصه مهم‌ها، هفته‌ای یک‌بار.
              </h2>
              <p className="mt-3 text-xs leading-6 text-black/60">
                تحلیل خبرهای هوش مصنوعی برای سازندگان، بدون شلوغی.
              </p>
              <Link
                href="/register"
      ...
      <section
        id="topics"
        className="home-reveal home-reveal-delay-3 scroll-mt-24"
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {topics.map((topic) => (
            <a
              key={topic.label}
              href="#latest"
              className="group flex min-h-36 flex-col justify-between rounded-[1.5rem] bg-white/[0.04] p-5 ring-1 ring-white/[0.06] transition-transform duration-700 ease-[cubic-bezier(0.32,0.72,0,1)] hover:-translate-y-1">
              ...
              <div className="flex items-end justify-between gap-4">
                ...
                <ArrowLeft
                  className="size-4 text-faint transition-transform duration-700 group-hover:-translate-x-1 group-hover:text-white"
                  strokeWidth={1.4}
                  aria-hidden="true"
                />
              </div>
        ...
        <div className="rounded-[calc(2rem-0.5rem)] bg-[#e8e4d8] px-6 py-10 shadow-[inset_0_1px_0_rgba(255,255,255,0.8)] sm:px-10 sm:py-12">
          <div className="grid gap-10 lg:grid-cols-[.7fr_1.3fr] lg:items-center">
            <div>
              ...
              <h2 className="balanced-text mt-4 text-4xl font-black leading-[1.35] tracking-[-0.045em]">
                خبر را بخوانید؛ بعد آن را امتحان کنید.
              </h2>
            </div>
              ...
              {[
                {
                  href: "/ai/analyzer",
                  label: "تحلیل پرامپت",
                  helper: "آماده برای مدل‌های تازه",
                  icon: Sparkles,
                },
                {
                  href: "/explore?type=prompts",
                  label: "پرامپت‌ها",
                  helper: "منتخب جامعه",
                  icon: Braces,
                },
                {
                  href: "/explore?type=skills",
                  label: "مهارت‌ها",
                  helper: "گردش‌کارهای واقعی",
                  icon: Shapes,
                },
              ].map((item) => {
                ...
                return (
                  <Link
                    key={item.href}
                    href={item.href}
                    className="group rounded-[1.25rem] bg-white/45 p-5 ring-1 ring-black/[0.06] transition-transform duration-700 ease-[cubic-bezier(0.32,0.72,0,1)] hover:-translate-y-1">
                    <Icon
                      className="size-5 text-[#496154]"
                      strokeWidth={1.4}
                      aria-hidden="true"
                    />
                    ...
                    <div className="mt-1 flex items-center justify-between gap-2 text-[10px] text-black/50">
                      ...
                      <ArrowLeft
                        className="size-3.5 transition-transform duration-700 group-hover:-translate-x-1"
                        strokeWidth={1.4}
                        aria-hidden="true"
                      />
                    </div>
