  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 xl:grid-cols-[330px_minmax(0,1fr)]">
        ...
        {selected && selectedId ? <ChatClient key={selectedId} conversationId={selectedId} currentUserId={user.id} participant={selected.participant} initialMessages={selected.messages} /> : <Card className="hidden min-h-[560px] place-items-center text-center xl:grid"><div><MessageCircle className="mx-auto size-9 text-faint" /><h2 className="mt-4 font-semibold">یک گفت‌وگو را انتخاب کنید</h2><p className="mt-2 text-sm text-muted">پیام‌های شما اینجا نمایش داده می‌شوند.</p></div></Card>}
      </div>
    </div>
  );
}
